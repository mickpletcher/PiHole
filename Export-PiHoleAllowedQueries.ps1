...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $scriptDirectory 'allowed_only_queries.csv'
}
...
$allowedStatusCodes = '2, 3, 12, 13, 14, 17'
...
SELECT
    ...
    status
FROM queries
WHERE status IN ($allowedStatusCodes)$timeFilter$domainWhereClause$clientWhereClause
ORDER BY timestamp;
"@.Trim()
